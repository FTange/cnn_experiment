...
def experiment_1layer_2cnn(x, y, layer1_nodes = [256],
                          dropout1 = [0.2], epochs = [10], filters1 = [32], 
                          filters2 = [32]
                          kernel_size = [(3, 3)], strides = [(1, 1)],
                          pool_size = [(2, 2)]):
    ...
    parameters = it.product(layer1_nodes, dropout1, epochs, filters1, filters2, kernel_size, strides, pool_size)

    for l1, drop1, epoch, num_filters1, num_filters2, k_size, stride, p_size in parameters:
        ...
        model_layers = [tf.keras.layers.Conv2D(num_filters, kernel_size = k_size, strides = stride, 
                        ...
                        tf.keras.layers.Conv2D(num_filters2, kernel_size = k_size, strides = stride, 
                                               activation = 'relu', input_shape = (15, 15, num_filters1)),
                        tf.keras.layers.MaxPooling2D(pool_size = p_size, padding='valid',
                                                     strides=None),
